<script>
  ...
  const tabs = [
    { icon: 'fas fa-table',   label: 'Table Storage Management', component: ManagementTab },
    { icon: 'fas fa-history', label: 'Table Audit Logs',          component: AuditLogsTab },
  ];
...
<div class="flex flex-col h-full bg-gray-50">
  ...
  <div class="flex items-center gap-3 px-6 py-4 bg-white border-b border-gray-200 flex-shrink-0">
    ...
    <span class="text-lg font-semibold text-gray-800">Table Storage Management</span>
  </div>
  ...
  <div class="flex border-b border-gray-200 bg-white px-4 flex-shrink-0">
    {#each tabs as tab, i}
      <button
        class={activeTab === i
          ? 'flex items-center px-4 py-3 text-sm font-medium border-b-2 border-emerald-600 text-emerald-600 cursor-pointer whitespace-nowrap'
